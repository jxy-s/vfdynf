#include <pch.h>
#include <hook_ole.hpp>

BSTR 
WINAPI
Hook_SysAllocString(
    _In_opt_z_ const OLECHAR * psz
    )
{
    if (layer::GetRecursionCount() == 0)
    {
        layer::RecursionGuard guard;

        if (fault::ShouldFaultInject(fault::Type::Ole))
        {
            return NULL;
        }
    }

    return thunks::CallOriginal(&Hook_SysAllocString,
                                thunks::g_OleAut32,
                                psz);
}

INT
WINAPI
Hook_SysReAllocString(
    _Inout_ _At_(*pbstr, _Pre_z_ _Post_z_ _Post_readable_size_(_String_length_(psz)+1)) BSTR* pbstr, 
    _In_opt_z_ const OLECHAR* psz
    )
{
    if (layer::GetRecursionCount() == 0)
    {
        layer::RecursionGuard guard;

        if (fault::ShouldFaultInject(fault::Type::Ole))
        {
            return NULL;
        }
    }

    return thunks::CallOriginal(&Hook_SysReAllocString,
                                thunks::g_OleAut32,
                                pbstr,
                                psz);
}

BSTR
WINAPI
Hook_SysAllocStringLen(
    _In_reads_opt_(ui) const OLECHAR * strIn, 
    UINT ui
    )
{
    if (layer::GetRecursionCount() == 0)
    {
        layer::RecursionGuard guard;

        if (fault::ShouldFaultInject(fault::Type::Ole))
        {
            return NULL;
        }
    }

    return thunks::CallOriginal(&Hook_SysAllocStringLen,
                                thunks::g_OleAut32,
                                strIn,
                                ui);
}

INT
WINAPI
Hook_SysReAllocStringLen(
    _Inout_ _At_(*pbstr, _Pre_z_ _Post_z_ _Post_readable_size_(len+1)) BSTR* pbstr, 
    _In_opt_z_ const OLECHAR* psz, 
    _In_ unsigned int len
    )
{
    if (layer::GetRecursionCount() == 0)
    {
        layer::RecursionGuard guard;

        if (fault::ShouldFaultInject(fault::Type::Ole))
        {
            return NULL;
        }
    }

    return thunks::CallOriginal(&Hook_SysReAllocStringLen,
                                thunks::g_OleAut32,
                                pbstr,
                                psz,
                                len);
}

BSTR
WINAPI
Hook_SysAllocStringByteLen(
    _In_opt_z_ LPCSTR psz, 
    _In_ UINT len
    )
{
    if (layer::GetRecursionCount() == 0)
    {
        layer::RecursionGuard guard;

        if (fault::ShouldFaultInject(fault::Type::Ole))
        {
            return NULL;
        }
    }

    return thunks::CallOriginal(&Hook_SysAllocStringByteLen,
                                thunks::g_OleAut32,
                                psz,
                                len);
}